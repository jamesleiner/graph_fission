...
ridge_estimates <-function(train,test,edge_mat,k=2,lambda_list = c(0.01,0.1,1,2,3,5,10,100)){
  mse_vec = c()
...
run_experiment_ridge <- function(graph,edges,edge_mat,k=0,cv=0.9,sd_level=1,tau=1,scale=10,err_type="normal",est_var=FALSE,K_list =c(2,3,4,5,6,7,8,9,10,15,20,25,30),lambda_list = c(1:100/1000,1:1000/10)){
  ...
  for(K in K_list){
    ...
    res = data.frame(type=c("1se","min","SURE"),
                     ...
                     nse =c(as.matrix(cbind(mse[ind_1se],mse[ind],mse[ind_sure]))),
...
res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=1,sd_level=1,tau=1),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_1_normal.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=1,sd_level=1,tau=1,est_var=TRUE),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_1_normal_estvar.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=1,sd_level=1,tau=1,err_type="sn"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_1_sn.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=1,sd_level=1,tau=1,err_type="t"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_1_t.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=1,sd_level=1,tau=1,err_type="laplace"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_1_laplace.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=2,sd_level=1,tau=1),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_2_normal.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=2,sd_level=1,tau=1,est_var=TRUE),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_2_normal_estvar.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=2,sd_level=1,tau=1,err_type="sn"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_2_sn.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=2,sd_level=1,tau=1,err_type="t"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_2_t.Rdata")

res=mclapply(1:ntrials, function(x) run_experiment2(graph,edges,edge_mat,k=2,sd_level=1,tau=1,err_type="laplace"),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
...
active_set = which(apply(edges,1, function(x) get_active(graph,x,x_sep=4))==1)
run_experiment_ridge(graph,edges,edge_mat,active_set,k=0,sd_level=1,tau=1)

active_set = which(apply(edges,1, function(x) get_active(graph,x,x_sep=4))==1)
res=mclapply(1:ntrials, function(x)run_experiment_ridge(graph,edges,edge_mat,active_set,k=0,sd_level=1,tau=1),mc.cores=128)
res = Reduce(function(dtf1, dtf2) rbind(dtf1, dtf2, by = "i", all.x = TRUE),res)
save(res,file="results_graphfission_crossval_0_ridge.Rdata")

load("results_graphfission_crossval_0_ridge.Rdata")
resload("results_graphfission_crossval_2_sn.Rdata")

label = "Gaussian Errors (w/ estimated variance)"
k=1
err_type = "normal_estvar"
...
active_set = c(1,27,30,43,69)

active_set = which(apply(edges,1, function(x) get_active(graph,x,x_sep=4))==1)


active_set = which(((graph[,1] == 5) &(graph[,2] < 5))| ((graph[,1] == 8) &(graph[,2] < 5)))


active_set = which(apply(edges,1, function(x) get_active(graph,x,x_sep=4))==1)
dat = generate_data(graph,edges,edge_mat, active_set,k=2,sd_level=1,scale=1,tau=1,est_var = TRUE)
...
sd_level = dat$sd_est
  ...
  for(K in K_list){
    ...
    lambda_list = c(0.01,0.02,0.04,1)  
...
lambda_list = c(1:100/1000,1:1000/10)
lambda_list = c(1,2,3)


run_experiment_ridge(graph,edges,edge_mat, active_set,k=0)



gram = diag(n) + lambda*n*t(penalty) %*% penalty

elastic_loss(beta_hat,dat$Y,edge_mat,k=k,lambda_1=0,lambda_2 =lambda)

beta_hat2 = optim(rep(0,100),function(x) elastic_loss(x,dat$Y,edge_mat,k=k,lambda_1=0,lambda_2 =lambda),method="L-BFGS")
...
run_experiment_elastic <- function(graph,edges,edge_mat, active_set,k=2,sd_level=1,scale=1,tau=1,err_type="normal",est_var=FALSE,lambda_list = c(0.01,0.1,1,2,3,5,10,100),K_list =c(2,3,4,5,6,7,8,9,10,15)){
  dat = generate_data(graph,edges,edge_mat, active_set,k=k,sd_level=sd_level,scale=scale,tau=tau,err_type=err_type,est_var=est_var)
  sd_level = dat$sd_est
...
resid = sapply(1:dim(h$beta)[2], function(x) h$beta[,x] - response)
mse = colMeans(resid**2)
sure = mse+2*h$df*sd_level/nrow(h$beta)
Z = rnorm(num,0,sd_est)


elastic_loss_grad <- function(beta,y,edge_mat,k=0,lambda_1=1,lambda_2=1){
  graph_laplacian <- t(edge_mat) %*% edge_mat
  if(k%%2 ==0 ) {
    penalty = edge_mat %*% matrix.power(graph_laplacian,k/2)
  }
  ...
  2*(beta - y) + lambda_1 * t(penalty) + 2*lambda_2 * t(penalty) %*% penalty
}
...
o <- optim(rep(0,100),function(x) elastic_loss(x,dat$Y,edge_mat,k=0,lambda_1=1,lambda_2 =2),method="L-BFGS")

df <- cbind(graph,o$par) 
...
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("tan", "tan"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "black", symbol = 104),showlegend=FALSE)
...
colnames(df2) <- c("x","y","z")
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("tan", "tan"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "blue", symbol = 104),showlegend=FALSE)

df2 <- cbind(graph,dat$g_Y) 
colnames(df2) <- c("x","y","z")
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("tan", "tan"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "green", symbol = 104),showlegend=FALSE)


CI <- get_CI(dat$f_Y,dat$g_Y,edge_mat,dat$mean,k=2,sd_level=4,K=5,cv=0.9)
df2 <- cbind(graph,dat$Y) 
colnames(df2) <- c("x","y","z")
