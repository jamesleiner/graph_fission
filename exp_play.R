...
dat = generate_data(graph,edges,edge_mat, active_set,k=0,sd_level=3,scale=6,tau=1,err_type="normal",est_var=TRUE)
select_df = dat$f_Y
infer_df = dat$g_Y
...
a = run_tf(dat$Y,edge_mat,k=0)
df2 <- cbind(graph,dat$Y) 
colnames(df2) <- c("x","y","z")
df <- cbind(graph,a$beta[,40])
colnames(df) <- c("x","y","z")
df <- df%>% pivot_wider(names_from =x, values_from = z)

colorscale = list(c(0, 1), c("tan", "blue"))
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("orange", "orange"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "black", symbol = 104),showlegend=FALSE)




a = run_tf(dat$Y,edge_mat,k=1)
df2 <- cbind(graph,dat$Y) 
colnames(df2) <- c("x","y","z")
df <- cbind(graph,a$beta[,50])
colnames(df) <- c("x","y","z")
df <- df%>% pivot_wider(names_from =x, values_from = z)

colorscale = list(c(0, 1), c("green", "blue"))
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("cyan", "cyan"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers" ,type = "scatter3d", marker = list(size = 5, color = "black", symbol = 104),showlegend=FALSE)




a = run_tf(dat$Y,edge_mat,k=2)
df2 <- cbind(graph,dat$Y) 
colnames(df2) <- c("x","y","z")
df <- cbind(graph,a$beta[,40])
colnames(df) <- c("x","y","z")
...
plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("lightgreen", "lightgreen"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "black", symbol = 104),showlegend=FALSE)

a =ridge_soln(dat$Y,edge_mat,k=0,lambda=0.005)
df2 <- cbind(graph,dat$Y) 
colnames(df2) <- c("x","y","z")
df <- cbind(graph,a$beta)
colnames(df) <- c("x","y","z")
df <- df%>% pivot_wider(names_from =x, values_from = z)

plot_ly(z=as.matrix(df)[,-1],type="surface",colorscale = list(c(0, 1), c("pink", "pink"))) %>%  hide_colorbar() %>% 
  add_trace(data = df2, x = ~x, y = ~y, z = ~z, mode = "markers", type = "scatter3d", marker = list(size = 5, color = "black", symbol = 104),showlegend=FALSE)


o <- optim(rep(0,100),function(x) elastic_loss(x,dat$Y,edge_mat,k=0,lambda_1=0.01,lambda_2 =0.005),method="L-BFGS")
...
filename = paste("results_graphfission_crossval_",k,"_","normal",".Rdata",sep="")
load(filename)
res2=res


filename = paste("results_graphfission_crossval_",k,"_","laplace",".Rdata",sep="")
load(filename)

aggregate(as.numeric(res$df) ~ res$K +res$method + res$split , FUN = mean)


aggregate(as.numeric(res2$df) ~ res2$K +res2$method + res2$split , FUN = mean)

aggregate(as.numeric(res_comb$df) ~ res_comb$K + res_comb$err_type + res_comb$method + res_comb$split , FUN = mean)


err_type_list  = c("normal","sn","normal_estvar")
k=0
label="Gaussian Errors (w/ known variance)"
...
df <- aggregate(as.numeric(res$df) ~ res$K + res$err_type + res$method + res$split , FUN = mean)
colnames(df) <- c('K',"errtype","method","split",'df')
df = df[df$split == "fission",]
df[df$method == "min",3] = "Minimum CV error"
df[df$method == "1se",3] = "1se rule"


cv_df <- df%>%
 ggplot( aes(x=as.numeric(K), y=df,color=as.factor(method),linetype=as.factor(errtype))) +
 geom_line(aes(color=as.factor(method),linetype=as.factor(errtype)), size = 1.5) +
