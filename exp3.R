...
get_CI <- function(select_df,infer_df,edge_mat,true_mean,cv=0.9,k=0,sd_level=1){
  ...
  lambda_min = h$lambda[which(sure == min(sure))]
...
run_experiment <- function(graph,edges,edge_mat,k=0,pct_active=0.3,cv=0.9,sd_level=1,tau=1){
  ...
  full_CI <- get_CI(dat$Y,dat$Y,edge_mat,dat$mean,cv=cv,k=k,sd_level=sd_level)
  fission_CI <- get_CI(dat$f_Y,dat$g_Y,edge_mat,dat$mean,cv=cv,k=k,sd_level=sd_level)
